...
// Count from the site's own content (the number of screenable signals).
const content = readFileSync(resolve(root, 'index.md'), 'utf-8')
const m = content.match(/(\d+)\s+signals/i)
if (!m) {
  console.error('gen-og-banner: could not find the signal count in index.md')
  process.exit(1)
}
const count = m[1]

let svg = readFileSync(resolve(here, 'og-banner.svg'), 'utf-8')
svg = svg.replace(/\d+ signals/g, `${count} signals`)
...
console.log(`gen-og-banner: wrote public/og-banner.webp (${count} signals)`)
